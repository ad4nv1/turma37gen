...
public class Projeto {
	...
	public static void main(String[] args) {
		...
		String loja[][] = { { "G1-1", "Iphone 5s", "750.0", "10", "0" }, { "G1-2", "Iphone6", "900.0", "10", "0" },
				{ "G1-3", "Iphone 7", "1500.0", "10", "0" }, { "G1-4", "Iphone 7 P", "1999.9", "10", "0" },
				{ "G1-5", "Iphone 8", "2300.0", "10", "0" }, { "G1-6", "Iphone 8 P", "2550.9", "10", "0" },
				{ "G1-7", "Iphone X", "3000.0", "10", "0" }, { "G1-8", "Iphone 11", "6500.0", "10", "0" },
				{ "G1-9", "Iphone 11 P", "7999.9", "10", "0" }, { "G1-10", "Iphone 12", "10000.0", "10", "0" } };

		String codCompra, confirmaCompra = "", validacao = "";
		char op;
		boolean loop = true;
		int opcaoPagamento = 0;
		double totalCompra = 0.0, qtdCompra = 0.0, valorCompra = 0.0, imposto = 0.0, valor10Porcento = 0.0,
				aumento15 = 0.0;
		...
		while (loop) {
			...
			for (int i = 0; i < 10; i++) {
				if (loja[i][4] != "0") {
					System.out.println(
							loja[i][0] + "  " + loja[i][1] + "  " + loja[i][2] + "  " + loja[i][3] + "  " + qtdCompra);
				}
			}
			...
			do {
				...
				if (validacao == "S") {
					...
					for (int i = 0; i < 10; i++) {
						if (codCompra.equals(loja[i][0])) {
							System.out.println(loja[i][0] + "\t" + loja[i][1] + "\t" + loja[i][2] + "\t" + loja[i][3]);
				...
				for (int i = 0; i < 10; i++) {
					if (loja[i][4] != "0") {
						System.out.println("COD: " + loja[i][0] + "  PRODUTO: " + loja[i][1] + "  " + loja[i][2]
								+ "  QTD ESTOQUE: " + loja[i][3] + "  QTD SELECIONADA: " + loja[i][4] + "\n");
					}
				}
			...
			for (int i = 0; i < 10; i++) {
				if (loja[i][4] != "0") {
					System.out.println(
							"  PRODUTO: " + loja[i][1] + "  PREÇO :" + loja[i][2] + "  QTD COMPRADA: " + loja[i][4]);
				}
			}
			System.out.println("------------------------------------------------------");
			...
			case 1:
				System.out.println("PAGAMENTO A VISTA, DESCONTO DE 10%, VALOR FINAL: " + (totalCompra - valor10Porcento));
				break;
			case 2:
				System.out.println("PAGAMENTO NO CARTÃO, ACRESCIMO DE 10%, VALOR FINAL: " + (totalCompra + valor10Porcento));
				break;
			case 3:
				System.out.println("PAGAMENTO NO CARTÃO EM 2X, ACRESCIMO DE 15%, VALOR DAS PARCELAS: "+ ((totalCompra + aumento15) / 2));
				break;
